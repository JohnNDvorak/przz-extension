...
\]
This is the \textbf{theoretical ceiling} --- the $K=3$ Levinson-Conrey method cannot do better.
}}
\end{center}

\textbf{Hierarchy of results:}
\begin{enumerate}
\item \textbf{The discovery:} $c = 1$ achieved at $R = 1.14978$ (Theorem~\ref{thm:saturation})
...
\begin{theorem}[Saturation of the Method --- $c = 1$ Achieved]
...
\begin{lemma}[Why $c \geq 1$]
\label{lem:c-floor}
The constant $c$ satisfies $c \geq 1$ because it is the ratio of the mollified second moment to the square of the first moment. By the Cauchy-Schwarz inequality:
\[
\left(\int_{T}^{2T} |\zeta \cdot \psi|^2 \, dt\right) \cdot \left(\int_{T}^{2T} 1^2 \, dt\right) \geq \left(\int_{T}^{2T} |\zeta \cdot \psi| \, dt\right)^2
\]
...
\begin{proof}
At $R = 1.14978$, we achieve $c = 1$ (Theorem~\ref{thm:saturation}), giving
$\kappa_{\text{main}} = 1$. The rigorous bound satisfies:
\[
\kappa_{\text{rigorous}}(T) \geq \kappa_{\text{main}} - \frac{C}{\log T}
= 1 - \frac{C}{\log T}
\]
where $C > 0$ is the explicit error constant from PRZZ~\cite{PRZZ2019}. Taking $T \to \infty$:
\[
...
\begin{tikzpicture}[
  ...
  % 1. PRZZ Baseline (Moved label to right to avoid y-axis clutter)
  ...
  % Critical Point on X-Axis
  % Used a tick mark instead of a floating arrow for precision
  \draw[thick, red!70!black] (2.3, 0) -- (2.3, -0.15);
  \node[below, red!70!black, font=\footnotesize] at (2.3, -0.15) {$R = 1.14978$};

  % Dashed projection to vertex
  \draw[dotted, red!70!black] (2.3, 0) -- (2.3, 1);
  \fill[red] (2.3,1) circle (2pt);

  % "Perfect Cancellation" Callout
  % Moved OUT of the parabola bowl to the top-right to avoid occlusion
  \draw[<-, thick, black] (2.35, 1.1) -- (3.5, 2.8)
      node[right, align=left, text width=4cm] {
  ...
  % X-axis (Manual ticks from original code)
  \draw (1, 0.1) -- (1, -0.1) node[below] {$0.5$};
  \draw (4, 0.1) -- (4, -0.1) node[below] {$1.5$};

\end{tikzpicture}
\caption{The curve $c(R)$ achieves its global minimum $c \approx 1$ at the critical value $R = 1.14978$. The PRZZ baseline (gray dashed) remains significantly higher at $c \approx 2.14$.}
...
Having established $c = 1$ at $R = 1.14978$ (Theorem~\ref{thm:saturation}), we examine the behavior as $T \to \infty$.
...
\begin{enumerate}
\item \textbf{Saturation:} $c = 1$ at $R = 1.14978$ (Theorem~\ref{thm:saturation})
