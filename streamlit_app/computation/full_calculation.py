...
def display_full_result(result: Dict):
    ...
    with col1:
        st.metric(
            label="κ (main)",
            value=f"{result['kappa']:.6f}",
        )

    with col2:
        if result.get('kappa_rigorous'):
            st.metric(
                label="κ (rigorous)",
                value=f"{result['kappa_rigorous']:.6f}",
            )
        else:
    ...
    st.markdown(
        f"$$c = {result['S12_plus']:.6f} + {result['m']:.4f} \\times "
        f"{result['S12_minus']:.6f} + {result['S34']:.6f} = {result['c']:.6f}$$"
    )
